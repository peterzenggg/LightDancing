...
namespace HidSharp.Platform.Windows
{
    sealed class WinHidStream : SysHidStream
    {
        ...
        internal void Init(string path,bool Priority)
        {
            IntPtr handle = NativeMethods.CreateFileFromDevice(path, 0, NativeMethods.EFileShare.Read | NativeMethods.EFileShare.Write);
            if (handle == (IntPtr)(-1))
            {
