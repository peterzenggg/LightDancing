...
namespace HidSharp.Platform.Windows
{
    sealed partial class WinHidDevice : HidDevice
    {
        ...
        protected override DeviceStream OpenDeviceDirectly(OpenConfiguration openConfig)
        {
            ...
            if (openConfig.IsOptionSet(OpenOption.Priority))
            {
                OpenPriority Priority = (OpenPriority)openConfig.GetOption(OpenOption.Priority);
                if (Priority == OpenPriority.High)
                {
                    try { stream.Init(_path,true); return stream; }
