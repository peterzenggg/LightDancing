...
{
    internal class RazerBlackwidowV3KeyboardController : ILightingControl
    {
        private const int DEVICE_VID = 0x1532;
        private const int DEVICE_PID = 0x024E;
        ...
        public List<USBDeviceBase> InitDevices()
        {
            ...
            if (streams != null && streams.Count > 0)
            {
                ...
                {
                    RazerBlackwidowV3KeyboardDevice keyboard = new RazerBlackwidowV3KeyboardDevice(stream);
    ...
    public class RazerBlackwidowV3KeyboardDevice : USBDeviceBase
    {
        private const int MAX_REPORT_LENGTH = 91;

        //private bool _isUIControl = false;

        public RazerBlackwidowV3KeyboardDevice(HidStream deviceStream) : base(deviceStream)
        {
        }
        ...
        protected override List<LightingBase> InitDevice()
        {
            ...
            RazerBlackwidowV3Keyboard keyboard = new RazerBlackwidowV3Keyboard(_model);
            hardwares.Add(keyboard);
        ...
        protected override HardwareModel InitModel()
        {
            ...
            return new HardwareModel()
            {
                FirmwareVersion = "NA",
                DeviceID = deviceID,
                USBDeviceType = USBDevices.RazerBlackwidowV3Keyboard,
    ...
    public class RazerBlackwidowV3Keyboard : LightingBase
    {
        /// <summary>
        ...
        public RazerBlackwidowV3Keyboard(HardwareModel hardwareModel) : base(KEYBOARD_YAXIS_COUNTS, KEYBOARD_XAXIS_COUNTS, hardwareModel)
        {
        }
        ...
        protected override LightingModel InitModel()
        {
            ...
            return new LightingModel()
            {
                ...
                Type = LightingDevices.RazerBlackwidowV3Keyboard,
        ...
        protected override void ProcessColor(ColorRGB[,] colorMatrix)
        {
            ...
            foreach (var ledList in COMMAND_LAYOUT)
            {
                byte[] result = new byte[MAX_REPORT_LENGTH];
                result[0] = 0x00; //report id
                result[1] = 0x08;
                result[2] = 0x3f;
                result[3] = 0x00;
                result[4] = 0x00;
                ...
                result[7] = 0x0f;
                result[8] = 0x03;
                result[11] = (byte)ledList.Key;
                result[13] = 0x15;

                for (int i = 0; i < ledList.Value.Count; i++)
                {
                    if (ledList.Value[i] != Keyboard.NA)
                    {
                        var position = KEYS_LAYOUTS[ledList.Value[i]];
                        var color = colorMatrix[position.Item1, position.Item2];
                        result[i * 3 + 17] = color.R;
                        result[i * 3 + 18] = color.G;
                        result[i * 3 + 19] = color.B;
                    }
                }

                result[89] = Methods.CalculateRazerAccessByte(result);
