...
{
    internal class Q60Controller : ILightingControl
    {
        private const string DEVICE_PID = "VID_3402&PID_0400";
        ...
        public List<USBDeviceBase> InitDevices()
        {
            ...
            if (streams != null && streams.Count > 0)
            {
                ...
                foreach (Tuple<SerialStream, string> stream in streams)
                {
                    ...
                    Q60Device device = new Q60Device(stream.Item1, stream.Item2);
    ...
    public class Q60Device : USBDeviceBase
    {
        ...
        private bool _isGettingDeviceData = true;
        private readonly FanGroup _thisQ60 = new FanGroup();
        private readonly object _lock = new object();
        private const int CHANNEL_COMMAND_INDEX = 12;
        private bool _firstInit = true;
        private bool _hotswap = true;
        public Q60Device(SerialStream deviceStream, string serialID) : base(deviceStream, serialID)
        {
        }

        protected override List<LightingBase> InitDevice()
        {
            /*Q60 Pump itself, only new when init*/
            Q60 q60Head = new Q60(this);
            _thisQ60.DeviceBases.Add(q60Head);
            _fanDevices = new List<FanGroup> { _thisQ60 };

            /*Q60 check for first 2 channel for smart devices*/
            //CountChannelDevices();
            CheckQ60BoardInfo();
        ...
        private void CountChannelDevices()
        {
            ...
            if (deviceChanged || _firstInit)
            {
                        ...
                        if(!_serialDevices[channel - 1][i].IsConnected || i == _serialDevices[channel - 1].Count - 1)
                        {
                            ...
                            else if (smartDevices[0].GetType() == typeof(FT12))
                            {
                                ...
                                foreach (var fan in smartDevices)
                                {
                                    ...
                                    if (channel == 2)
                                    {
                                        ((FT12)fan).WindDirection = ((Q60)_thisQ60.DeviceBases[0]).WindDirection;
                                    }
                                }
                                _fanDevices.Add(fanGroup);
                                Debug.WriteLine("ft12 group added");
                                smartDevices.Clear();
                            }
                        }
                    }
                ...
                /*Port 3 - LCD Backlight*/
                Q60LCDBacklight lcdBacklight = new Q60LCDBacklight("3_1", _model);
                _hardwares.Add(lcdBacklight);
                _lightingBases[2].Add(lcdBacklight);

                /*Port 4 - NP50 Logo*/
                Q60Logo q60logo = new Q60Logo("4_1", _model);
                _hardwares.Add(q60logo);
                _lightingBases[3].Add(q60logo);
        ...
        private void CheckQ60BoardInfo()
        {
            if (_isFirstRead)
            {
                ...
                {
                    //Console.WriteLine("Fail to check Q60 info.");
                }
            ...
            catch { Console.WriteLine("Fail to check Q60 info."); }
            Thread.Sleep(100);
            ...
            catch { Console.WriteLine("Fail to read Q60 info."); }
            ...
            if (data[0] == 0xFF && data[1] == 0xCC)
            {
                ...
                WindDirection windDirection = (WindDirection)data[11];

                ((Q60)_thisQ60.DeviceBases[0]).Noise = noise;
                ((Q60)_thisQ60.DeviceBases[0]).PumpTempIn = internalTemp;
                ((Q60)_thisQ60.DeviceBases[0]).PumpTempOut = pumpTemp;
                ((Q60)_thisQ60.DeviceBases[0]).CurrentRPM = pumpRpm;
                ((Q60)_thisQ60.DeviceBases[0]).WindDirection = windDirection;
            }
            else
            {
                //Console.WriteLine("Fail to read info from Q60");
            }
        }
        ...
        private SmartDevice CreateNewDeviceObject(byte deviceType, byte isConnected, int channel, int fanCount)
        {
            SmartDevice device = deviceType switch
            {
                1 => new PQ10(),
                2 => new PQ30(),
                3 => new FT12(this, "Q60 Channel" + channel + "Fan" + fanCount),
                _ => null,
            };
        ...
        private byte[] SendGetInfoCommands(int channel)
        {
            ...
            catch { Console.WriteLine("Fail to check Q60 channel " + channel + " info."); }
            Thread.Sleep(100);
            ...
            catch { Console.WriteLine("Fail to read Q60 channel " + channel + " info."); }
        ...
        public void SetFanSpeed()
        {
            ...
            for (int channel = 1; channel <= 2; channel++)
            {
                ...
                {
                    //Console.WriteLine("Fail to set fan speed of Q60 channel " + channel);
                }
            }
        }
        ...
        public void SetPumpSpeed()
        {
            ...
            commands[3] = 0x00;
            commands[4] = 0x00; //on
            commands[5] = (byte)((Q60)_fanDevices.FirstOrDefault().DeviceBases[0]).SpeedPercentage;
            ...
            {
                //Console.WriteLine("Fail to set Q60 pump speed");
            }
        }
        ...
        public void SetMBBypass()
        {
            ...
            {
                //Console.WriteLine("Fail to set Q60 to MB mode");
            }
        }
