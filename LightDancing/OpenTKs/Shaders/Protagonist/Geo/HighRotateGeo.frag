...
float high_square(vec2 _st, float size, vec2 center, bool left){
    float fill = 0.0;
...
float high_quarter(vec2 _st, float size, vec2 center){
    vec2 position = center + vec2(0.5) * size;
    float fill = high_square(_st, size, position, true);
    position = center + vec2(0.5, -0.5) * size;
    fill += high_square(_st, size, position, false);
    position = center + vec2(-0.5, -0.5) * size;
    fill += high_square(_st, size, position, true);
    position = center + vec2(-0.5, 0.5) * size;
    fill += high_square(_st, size, position, true);
    return fill;    
}
...
vec4 HighRotateGeo(){
    ...
    for(int x = 0; x < 4; x++){
        ...
        for(int i = 1; i < 2; i++){
            float size = pow(0.5 , float(i))  / 2.0 * (high_intensity + 1.0);
            vec2 center = vec2(0.5, 0.5) + vec2(-1.0, 1.0) * size;

            float draw = high_quarter(rotate_st, size, center);
            if(draw != 0.0)
