...
float high_line(vec2 _st, vec2 position, float size){
...
vec4 HighHexagon() {
    ...
    st -= 0.5;
    st *= high_rotate2d(high_u_time + u_time);
    st += 0.5;
    vec3 color = vec3(0.);
    ...
    rotate_st *= high_rotate2d(-rotateA);
    rotate_st += 0.5;
    for(int i = 0; i < 10; i++){
    	...
    	color += high_line(rotate_st, position, 0.05) * high_displayColor;
    }
