...
vec3 BG_square(vec2 _st, float size, vec2 center, bool left){
    float fill = 0.0;
...
vec3 BG_quarter(vec2 _st, float size, vec2 center){
    vec3 display = vec3(0.0);
    vec3 result = BG_square(_st, size, center, true);
    if(result != vec3(0.0)){
        display = result;
    }
    
    result = BG_square(_st, size * 0.5, center, false);
    if(result != vec3(0.0)){
        display = result;
    }
...
vec4 GeoSquare7(){
    ...
    for(int x = 0; x < 4; x++){
        ...
        for(int i = 1; i < 4; i++){
            ...
            vec3 draw = BG_quarter(rotate_st, size, center);
            if(draw != vec3(0.0))
                color = draw;
    ...
    float size = pow(0.5 , float(1))  / 2.0;
            ...
            vec3 draw = BG_quarter(st, size, center);
            //color = draw;
