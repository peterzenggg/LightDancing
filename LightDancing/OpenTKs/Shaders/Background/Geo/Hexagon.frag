...
float BG_line(vec2 _st, vec2 position, float size){
...
vec4 Hexagon() {
    ...
    st -= 0.5;
    st *= BG_rotate2d(low_u_time + u_time);
    st += 0.5;
    vec3 color = vec3(0.);
    ...
    for(int i = 0; i < 10; i++){
    	...
    	color += BG_line(rotate_st, position, 0.05) * low_displayColor;
    }
