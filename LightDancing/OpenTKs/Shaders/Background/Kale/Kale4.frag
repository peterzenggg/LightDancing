...
vec4 Kale4(){
    ...
    if(mod(index, 2.0) == 0.0){
            kale_st.x = length * cos(mod(a, 2.0 * PI / sides)) * 2.0;
            kale_st.y = length * sin(mod(a, 2.0 * PI / sides)) * 2.0;    
    }
	...
	for(int i = 0; i < 10; i++){
        ...
        float y =  size;
        vec2 still = kale_st;
        kale_st = mix(vec2(low_noise(kale_st + (u_time + low_u_time / 2.0) / 25.0)), kale_st, sin((u_time + low_u_time) / 15.0) + cos((u_time) / 1.5)); 
        float draw = smoothstep(y - 0.08, y, fract(kale_st.x + 0.5)) - smoothstep(y, y + 0.08, fract(kale_st.x + 0.25));
        if(draw != 0.0)
            color += draw * displayColor * 0.2;
    }
