...
vec4 Kale1(){
	...
	for(int i = 0; i < 10; i++){
        ...
        float y = size;
        vec2 still = kale_st;
        kale_st = mix(vec2(low_noise(kale_st + (u_time + low_u_time / 4.0) / 20.0)), kale_st, sin((u_time) / 5.0) * 0.5 + cos((u_time) / 8.5)); 
        float draw = smoothstep(y, y + 0.3, fract(kale_st.x)) - smoothstep(y + 0.3, y + 0.6, fract(kale_st.x));
        if(draw != 0.0)
            color += draw * displayColor * 0.3;
    }
