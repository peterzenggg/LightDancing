...
vec4 Kale11(){
	...
	for(int i = 0; i < 10; i++){
        ...
        vec2 still = kale_st;
        kale_st = mix(vec2(low_noise(kale_st + low_u_time / 40.0)), kale_st, sin((u_time) / 35.0) + cos((u_time) / 7.5)); 
        kale_st += 0.1;
        float draw = smoothstep(y - 0.1, y, fract(kale_st.x)) - smoothstep(y, y + 0.1, fract(kale_st.x));
        if(draw != 0.0)
