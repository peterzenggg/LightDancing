...
vec4 Kale12(){
    ...
    if(mod(index, 2.0) == 0.0){
            kale_st.x = length * cos(mod(a, 2.0 * PI / sides)) * 4.0;
            kale_st.y = length * sin(mod(a, 2.0 * PI / sides)) * 4.0;    
    }
	...
	for(int i = 0; i < 10; i++){
        ...
        float y = size;
        vec2 still = kale_st;
        kale_st = vec2(low_noise(kale_st + (u_time + low_u_time / 4.0) / 10.0));
        float draw = smoothstep(y - 0.4, y, kale_st.x) - smoothstep(y, y + 0.4, kale_st.x);
        if(draw != 0.0)
