...
vec4 AlienFlower2(){
    ...
    for(int i = 0; i < 8; i++){
        ...
        if(mod(float(i), 3.0) == 0.0)
            ...
            displayColor = vec3(1.000,0.305,0.880);
        else
            displayColor = vec3(0.001,0.769,1.000);
        
        float size = 1.0 / 8.0 * float(8 - i);
        float wave = 0.01 * float(8 - i) + 0.02;
        float y =  sin(index) * wave * low_intensity + size + noise(st * 5.0) * -0.436 ;
        float fill = step(length, y);
        if(fill != 0.0)
            color = displayColor;
    }
    
    vec3 et = low_alien(rotateSt, vec2(0.5));
    if(et != vec3(0.0))
        color = et;
