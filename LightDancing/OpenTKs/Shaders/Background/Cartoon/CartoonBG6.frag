...
vec4 CartoonBG6() {
    ...
    float index = a / (2.0 * PI) * 10.0;
    
    float y = fract(index) * 0.1 * low_intensity;
        ...
        y =  fract(index) * 0.2 * low_intensity;
        fill = step(length, y);
        if(color == vec3(0.0)){
        ...
        y = fract(index) * 0.3 * low_intensity;
        fill = step(length, y);
        if(color == vec3(0.0)){
        ...
        y = fract(index) * 0.4 * low_intensity;
        fill = step(length, y);
        if(color == vec3(0.0)){
        ...
        y = fract(index) * 0.5 * low_intensity;
        fill = step(length, y);
        if(color == vec3(0.0)){
