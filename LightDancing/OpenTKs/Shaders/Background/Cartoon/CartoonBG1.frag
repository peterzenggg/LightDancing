...
vec4 CartoonBG1() {
    ...
    float index = a / (2.0 * PI) * 3.14 * 20.0;

    float y = sin(index) * 0.01 * low_intensity + 0.1 * low_intensity;
        ...
        y = sin(index) * 0.015 * low_intensity + 0.2 * low_intensity;
        fill = step(length, y);
        if(color == vec3(0.0)){
        ...
        y = sin(index) * 0.02 * low_intensity + 0.3 * low_intensity;
        fill = step(length, y);
        if(color == vec3(0.0)){
        ...
        y = sin(index) * 0.025 * low_intensity + 0.4 * low_intensity;
        fill = step(length, y);
        if(color == vec3(0.0)){
        ...
        y = sin(index) * 0.03 * low_intensity + 0.5 * low_intensity;
        fill = step(length, y);
        if(color == vec3(0.0)){
